...
import { MoreHorizontal, Edit, Plus, Trash2, Search, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight, Inbox, CalendarOff } from 'lucide-react';
import { tipoAusentismoService } from '@/services/tipoausentismo.service';
import type { TipoAusentismo } from '@/types/interfaces';
import { useToast } from '@/hooks/use-toast';
import { useState, useMemo } from 'react';

interface TipoAusentismoTableProps {
  records: TipoAusentismo[];
  isLoading: boolean;
  onEdit: (record: TipoAusentismo) => void;
  onAddNew: () => void;
}

const PAGE_SIZE_OPTIONS = [10, 15, 20];

export default function TipoAusentismoTable({ records, isLoading, onEdit, onAddNew }: TipoAusentismoTableProps) {
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(PAGE_SIZE_OPTIONS[0]);
  const [filter, setFilter] = useState('');
  const [pendingDelete, setPendingDelete] = useState<TipoAusentismo | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter(r => (r.nombre_tipo_ausentismo || '').toLowerCase().includes(f) || String(r.codigo_tipo_ausentismo).includes(f));
  }, [records, filter]);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await tipoAusentismoService.delete(pendingDelete.codigo_tipo_ausentismo);
      window.dispatchEvent(new CustomEvent('records-changed'));
      toast({ title: 'Tipo eliminado', description: `"${pendingDelete.nombre_tipo_ausentismo}" se eliminó correctamente.` });
    } catch (err) {
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white shadow-sm">
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? renderSkeleton() : paginated.map(record => (
                <TableRow key={record.codigo_tipo_ausentismo} className="hover:bg-indigo-50/40">
                  <TableCell className="font-medium text-gray-900">{record.codigo_tipo_ausentismo}</TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2 text-gray-700">
                      <CalendarOff className="h-3.5 w-3.5 text-gray-400" />
                      {record.nombre_tipo_ausentismo}
                    </div>
                  </TableCell>
                  <TableCell>
      ...
      <AlertDialog open={!!pendingDelete} onOpenChange={(open) => !open && setPendingDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>¿Eliminar este tipo de ausentismo?</AlertDialogTitle>
            <AlertDialogDescription>
              Esta acción eliminará &quot;{pendingDelete?.nombre_tipo_ausentismo}&quot; y no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
