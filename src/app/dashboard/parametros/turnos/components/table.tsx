...
import { MoreHorizontal, Edit, Plus, Trash2, Search, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight, Inbox, Clock } from 'lucide-react';
import { turnoService } from '@/services/turno.service';
import type { Turno } from '@/types/interfaces';
...
interface TurnoTableProps {
  records: Turno[];
  isLoading: boolean;
  onEdit: (record: Turno) => void;
  onAddNew: () => void;
}

const PAGE_SIZE_OPTIONS = [10, 15, 20];

export default function TurnoTable({ records, isLoading, onEdit, onAddNew }: TurnoTableProps) {
  const [page, setPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(PAGE_SIZE_OPTIONS[0]);
  const [filter, setFilter] = useState('');
  const [pendingDelete, setPendingDelete] = useState<Turno | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter(r => (r.nombre_turno || '').toLowerCase().includes(f) || String(r.codigo_turno).includes(f));
  }, [records, filter]);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await turnoService.delete(pendingDelete.codigo_turno);
      window.dispatchEvent(new CustomEvent('records-changed'));
      toast({ title: 'Turno eliminado', description: `"${pendingDelete.nombre_turno}" se eliminó correctamente.` });
    } catch (err) {
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? renderSkeleton() : paginated.map(record => (
                <TableRow key={record.codigo_turno} className="hover:bg-indigo-50/40">
                  <TableCell className="font-medium text-gray-900">{record.codigo_turno}</TableCell>
                  <TableCell>
      ...
      <AlertDialog open={!!pendingDelete} onOpenChange={(open) => !open && setPendingDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción eliminará el turno &quot;{pendingDelete?.nombre_turno}&quot; y no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
